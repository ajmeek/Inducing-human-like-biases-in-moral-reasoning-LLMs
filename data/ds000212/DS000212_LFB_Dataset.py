...
from datasets import IterableDataset, Features, Value, DatasetInfo, Sequence
...
def load_LFB_dataset(basedir : Path, sampling_method=Sampling.LAST, subject=None):
    ...
    def _process_tsv(from_tsv: Path):
        ...
        for row in rows:
            ...
            label = [row[k] for k in ('condition', 'item', 'key')]
            labels.append(label)
    ...
    wdataset = wds.WebDataset(tarfiles, nodesplitter=wds.shardlists.split_by_worker).decode("pil").compose(_get_samples)
    features = Features(
        {
            "label": Sequence(feature=Value(dtype='float32'), length=DATA_LENGTH, id=None),
