...
def load_np_fmri_to_tensor(base_path: str,
    ...
    """
    Load the fmri data from the npy files and return the tokens, masks,
    and target tensors. The structure of the data is as follows: the
    base_path contains a directory for each subject. Each subject directory
    contains several files labeled labels-*.npy and *.npy.
    :param num_samples: the number of samples to load. Note that the current
        numpy matrix will always be loaded in full.
    ...
    for directory in os.listdir(base_path):
        for file in os.listdir(os.path.join(base_path, directory)):
            if file.endswith('.npy') and file.startswith('labels'):
                fmri = np.load(os.path.join(base_path, directory,
                                            file.replace('labels-', '')))
                text = np.load(
                    os.path.join(base_path, directory, file)).tolist()
    ...
    if normalize:
        target_tensors = \
            (target_tensors - target_tensors.mean()) / target_tensors.std()
    return tokens, masks, [target_tensors]
...
def load_ds000212_dataset(
    ...
    assert len(scenarios) == len(
        fmri_items), f'Expected: {len(scenarios)} == {len(fmri_items)}'
    # Pad with 0 those smaller one
    max_l = max(e.shape[0] for e in fmri_items)
    fmri_items = [F.pad(e, (0, max_l - e.shape[0]), 'constant', 0) for e in
                  fmri_items]
    ...
    # indices = [i for i, e in enumerate(fmri_items) if len(e) == most_common_len]
    ...
    return tokens[random_indices], masks[random_indices], target_tensors[
        random_indices]


def multiple_dataset_loading(datapath, tokenizer, config) -> \
        tuple[list[DataLoader], list[DataLoader], list[Union[int, tuple[int]]]]:
    train_head_dims = []
    dataloaders_train = []
    dataloaders_val = []
    for index, train_dataset in enumerate(config['train_datasets']):
        if train_dataset == 'ds000212':
            tokens, masks, target = load_ds000212_dataset(
                datapath, tokenizer, config['num_samples_train'],
                normalize=config['normalize_fmri'])

            dataloader_train, dataloader_val = preprocess(
                tokens, masks, target, config)

            dataloaders_train.append(dataloader_train)
            dataloaders_val.append(dataloader_val)
            train_head_dims.append(target.shape[1])

        elif train_dataset.startswith('ethics'):
            tokens, masks, target = load_csv_to_tensors(
