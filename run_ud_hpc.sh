...
# This is for using University of Delaware's Caviness HPC system.
# Conda is required for it so this must be changed to use conda instead of pip.
# Assume the environment is already activated, as done in the batch job submission script serial.qs
...
function provision() {
  ...
  python3 -m conda install -c conda-forge datalad-installer
...
#    pipenv run python3 "$root_dir/src/main.py" "$@"
  conda python3 "$root_dir/src/main.py" "$@"
}
